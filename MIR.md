...
## MIR program
   ...
   * API functions `MIR_write (MIR_context_t ctx, FILE *f)` and
     ...
     **binary MIR representation** to/from given file.  There are also
     ...
     (*writer_func) (void *, uint8_t), void *writer_arg)` and `MIR_read_with_func
     (MIR_context_t ctx, const int (*reader_func) (void *), void *reader_arg)` to
     ...
     * `reader_arg` or `writer_arg` is an arbitrary user-defined pointer that
       is passed verbatim to the first argument of the callback function.
    ...
    * `MIR_link` function also sets up call interface
	...
	only on their first execution
      * If you pass non-null `import_resolver` function, it will be
        ...
        The function get the import name from 2nd parameter and return the address which
        ...
        `resolver_arg` is an arbitrary user-defined pointer that is passed
        verbatim into the 1st parameter of the callback function.
